...
module.exports = function (helper) {
    ...
    return {
        configure: function (options) {
            ...
            typeof options.includeYM !== 'undefined' || (options.includeYM = true);

            helper.prebuild(function (magic) {
                return scanner.scan(options.levels, options.schemeBuilder)
                    .then(function (files) {
                        ...
                        return vow.all(Object.keys(nodes).map(function (node) {
                            ...
                            if (options.langs) {
                                decl.unshift({ block: 'i18n' });
                            }

                            bemdeclSource = 'exports.deps = ' + JSON.stringify(decl) + ';';
            ...
            helper.configure(function (projectConfig) {
                options.onStart && (startedResult = options.onStart());
                configurator.configure(projectConfig, options);
            });

            helper._projectConfig.task(helper.getTaskName(), function () {
                options.onFinish && options.onFinish(startedResult);
                if (helper.getMode() === 'pre' || process.env.MAKE_ONLY) {
                    return;
                }

                const toRun = Object.keys(nodesToRun);
                const runner = options.runner;

                return toRun.length && runner && runner.run(toRun, root, sources);
            });
        }
    };
};
