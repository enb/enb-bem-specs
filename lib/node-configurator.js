var path = require('path'),
    ...
    templateEngineDefault = {
        templateTech: 'enb-bemxjst/techs/bemhtml',
        templateOptions: {
            compat: true
        },
        htmlTech: 'enb-bemxjst/techs/bemjson-to-html',
        htmlTechOptionNames: { bemjsonFile: 'bemjson', templateFile: 'bemhtml' },
    },
...
exports.configure = function (config, options) {
    ...
    templateEngine.templateTech = require(templateEngine.templateTech);
    templateEngine.htmlTech = require(templateEngine.htmlTech);

    var TEMPLATE_ENGINE_SUFFIX = templateEngine.htmlTechOptionNames.templateFile,
        templateEngineOpts = _.assign({}, templateEngine.templateOptions, {
        target: '?.browser.' + TEMPLATE_ENGINE_SUFFIX + '.js',
        filesTarget: '?.' + TEMPLATE_ENGINE_SUFFIX + '.files',
    });
    ...
    config.nodes(pattern, function (nodeConfig) {
        ...
        nodeConfig.addTechs([
            [depsByTechToBemdecl, {
                target: '?.js.' + TEMPLATE_ENGINE_SUFFIX + '.bemdecl.js',
                sourceTech: 'js',
                destTech: 'bemhtml'
            }],
            [depsByTechToBemdecl, {
                target: '?.spec-js.' + TEMPLATE_ENGINE_SUFFIX + '.bemdecl.js',
                sourceTech: 'spec.js',
                destTech: 'bemhtml'
            }],
            [mergeBemdecl, {
                target: '?.' + TEMPLATE_ENGINE_SUFFIX + '.bemdecl.js',
                sources: [
                    '?.js.' + TEMPLATE_ENGINE_SUFFIX + '.bemdecl.js',
                    '?.spec-js.' + TEMPLATE_ENGINE_SUFFIX + '.bemdecl.js',
                    '?.bemjson.bemdecl.js'
                ]
            }],

            [depsTech, {
                target: '?.' + TEMPLATE_ENGINE_SUFFIX + '.deps.js',
                bemdeclFile: '?.' + TEMPLATE_ENGINE_SUFFIX + '.bemdecl.js'
            }],
            [files, {
                depsFile: '?.' + TEMPLATE_ENGINE_SUFFIX + '.deps.js',
                filesTarget: '?.' + TEMPLATE_ENGINE_SUFFIX + '.files',
                dirsTarget: '?.' + TEMPLATE_ENGINE_SUFFIX + '.dirs'
            }],
        ...
        nodeConfig.addTechs([
            ...
            [mergeFiles, {
                target: '?.pre.spec.js',
                sources: ['?.pure.ym.js', '?.source.browser.js', '?.browser.' + TECH_SUFFIX + '.js', '?.pure.spec.js']
