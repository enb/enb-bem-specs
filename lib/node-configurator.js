...
exports.configure = function (config, options) {
    ...
    var extendedLevels = [].concat(options.sourceLevels);
    var corePattern = path.join('bem-core', 'common.blocks');
    var hasCore = false;

    extendedLevels.forEach(function (level) {
        var levelPath = (typeof level === 'string') ? level : level.path;

        if (levelPath.indexOf(corePattern) !== -1) {
            hasCore = true;
        }
    });

    extendedLevels.unshift(path.join(libsDir, 'bem-pr', 'spec.blocks'));

    if (!hasCore) {
        extendedLevels.unshift(path.join(libsDir, 'bem-core', 'common.blocks'));
    }
    ...
    config.nodes(pattern, function (nodeConfig) {
        ...
        // Deps
        nodeConfig.addTechs([
            [depsByKeeps, { target: '?.base.bemdecl.js' }],
